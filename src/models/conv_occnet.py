...
class LocalPoolPointnet(nn.Module):
    ...
    def __init__(
        ...
        self.fc_pos = nn.Linear(inf_dim, 2*ef_dim)
        self.blocks = nn.ModuleList([
            ResnetBlockFC(2*ef_dim, ef_dim) for i in range(n_blocks)
        ])
        self.fc_c = nn.Linear(ef_dim, zf_dim)

        self.actvn = nn.ReLU()
        self.hidden_dim = ef_dim
    ...
    def generate_grid_features(self, p, c):
        ...
        fea_grid = c.new_zeros(p.size(0), self.zf_dim, self.reso_grid**3)
        c = c.permute(0, 2, 1)
        fea_grid = scatter_mean(c, index, out=fea_grid) # B x C x reso^3
        fea_grid = fea_grid.reshape(p.size(0), self.zf_dim, self.reso_grid, self.reso_grid, self.reso_grid) # sparce matrix (B x 512 x reso x reso)
